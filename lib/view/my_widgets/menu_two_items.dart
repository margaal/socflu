...
class MenuTwoItems extends StatelessWidget{
  ...
  MenuTwoItems({@required String this.item1, @required String this.item2, @required PageController this.pageController});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      width: 300.0,
      height: 50.0,
      decoration: BoxDecoration(color: pointer, borderRadius: BorderRadius.all(Radius.circular(50.0))),
      child: CustomPaint(
        painter: MyPainter(pageController),
        child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
        children: <Widget>[
          itemButton(item1), itemButton(item2)
        ],
      ),
      )
    );
